...
const IMPLEMENTER_VARIANT = "xhigh";
            ...
            findings: [
                {
                    ...
                    file: null,
                    line: null,
                    fix: `Architect review output was not parseable: ${error instanceof Error ? error.message : String(error)
                        }`,
                },
            ],
        };
...
${nonBlocking.length > 0
            ? nonBlocking
                .map(
                    ({ slice, finding }) =>
                        `- #${slice.number} ${finding.file ?? "general"}${finding.line ? `:${finding.line}` : ""
                        } - ${finding.fix}`,
                )
                .join("\n")
            : "None"
...
function formatFindings(findings: ReviewFinding[]) {
    ...
    return findings
        .map(
            (finding) =>
                `- ${finding.severity}: ${finding.file ?? "general"}${finding.line ? `:${finding.line}` : ""
                } - ${finding.fix}`,
        )
        .join("\n");
}
