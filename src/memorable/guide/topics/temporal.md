...
Use correction when the old record was wrong, misleading, or stapled to the wrong Entity. Call `memorable_correct` with `space`, `record_type`, `record_id`, `source`, `at`, and either `new_statement`, `about`, or both. Correction fixes the existing record instead of creating a ghost version that was never true.

Use invalidation when a claim stopped being true and there is no successor. Call `memorable_invalidate` with `space`, `record_type`, `record_id`, and `at`; the record becomes invalidated and gains an Invalidation Time.
