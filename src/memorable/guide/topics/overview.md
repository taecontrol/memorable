...
Markdown summaries, reviews, plans, and reports are Generated Views. They can be useful outputs from memory, but they are not the canonical store unless an agent intentionally writes their contents back as structured memory.

Memorable is for current truth, point-in-time truth, explainable history, and inspectable project context. It is not a file index, a Markdown folder with search, one global memory, or storage-specific API.
